...
class Supervised:
    ...
    def train(self, dataloader, operator, epochs, lr=0.0001, wd=1e-8):
        ...
        filename = 'supervised_{}_{}'.format(self.net, datetime.now().strftime('%d%m%Y'))
        log = Log(self.log_path, filename, ['epoch', 'loss', 'psnr', 'mse', 'ssim', 'gpu_memory'])
    ...
    def _save_model(self, epoch, model, optimizer):
        ...
        torch.save(state, os.path.join(self.trained_model_path, 'supervised_{}_{}.pth.tar'.format(self.net, datetime.now().strftime('%d%m%Y'))) )

    def _closure(self, net, dataloader, operator,
                        optimizer, mse):
