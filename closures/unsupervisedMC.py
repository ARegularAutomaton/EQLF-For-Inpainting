...
class MCUnsupervised:
    ...
    def train(self, dataloader, operator, epochs, lr=0.0001, wd=1e-8):
        ...
        filename = 'mc_unsupervised_{}_{}'.format(self.net, datetime.now().strftime('%d%m%Y'))
        log = Log(self.log_path, filename, ['epoch', 'loss_mc', 'psnr', 'mse', 'ssim', 'gpu_memory'])
    ...
    def _print_stats(self, epoch, epochs, loss):
        print('{}\tEpoch[{}/{}]\tmc={:.4e}'.format(datetime.now().strftime('%y-%m-%d-%H:%M:%S'), epoch+1, epochs, *loss))
    
    def _save_model(self, epoch, model, optimizer):
        state = {'epoch': epoch,
                'net': model.state_dict(),
                'optimizer': optimizer.state_dict()}
        torch.save(state, os.path.join(self.trained_model_path, 'mc_{}_{}.pth.tar'.format(self.net, datetime.now().strftime('%d%m%Y'))) )

    def _closure(self, net, dataloader, operator, optimizer, mse):
        loss_mc_seq, psnr_seq, mse_seq, ssim_seq, gpu_memory_seq = [], [], [], [], []
        ...
        loss_closure = [np.mean(loss_mc_seq), np.mean(psnr_seq), np.mean(mse_seq), np.mean(ssim_seq), mean(gpu_memory_seq)]
        return loss_closure
