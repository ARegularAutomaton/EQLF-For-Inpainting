...
class EISupervised:
    ...
    def train(self, dataloader, operator, transform, epochs, lr=0.0001, wd=1e-8):
        ...
        ei = torch.nn.MSELoss().to(self.device)
        optimizer = Adam(model.parameters(), lr=lr, weight_decay=wd)

        filename = 'ei_supervised_{}_{}'.format(self.net, datetime.now().strftime('%d%m%Y'))
    ...
    def _save_model(self, epoch, model, optimizer):
        ...
        torch.save(state, os.path.join(self.trained_model_path, 'ei_supervised_{}_{}.pth.tar'.format(self.net, datetime.now().strftime('%d%m%Y'))) )

    def _closure(self, net, dataloader, operator, transform, optimizer, mse, ei):
        loss_x_seq, loss_ei_seq, loss_seq, psnr_seq, mse_seq, ssim_seq, gpu_memory_seq = [], [], [], [], [], [], []
        for _, x in enumerate(dataloader):
            ...
            if self.problem == 'inpaint':
                ...
                y0 = operator.A(x)
                x0 = net(operator.A_dagger(y0))

                x1 = transform.apply(x0)
                x2 = net(operator.A_dagger(operator.A(x1)))

            elif self.problem == 'ct':
                def to_device(x):
                    return x.unsqueeze(1).type(self.dtype).to(self.device)
                def to_cpu(x):
                    return x.squeeze(1).detach().to('cpu')
                if len(x.shape)==4:
                    x = x.squeeze(1)

                angles = operator.get_angles()
                y0, proj_id = operator.A(x, add_noise=True, angles=angles)
                x0 = net(to_device(operator.A_dagger(y0, proj_id=proj_id, algorithm='FBP')))
                x1 = transform.apply(x0)
                tmp, proj_id = operator.A(to_cpu(x1), angles=angles, add_noise=False)
                x2 = net(to_device(operator.A_dagger(tmp, proj_id=proj_id, algorithm='FBP')))
                x = x.type(self.dtype).to(self.device)

            loss_x = mse(x0, x)
            loss_ei = ei(x1, x2)
            loss = loss_x + 1 * loss_ei
            ...
            ssim_seq.append(cal_ssim(x0,x.squeeze()))
        ...
        loss_closure = [np.mean(loss_x_seq), np.mean(loss_ei_seq), np.mean(loss_seq), np.mean(psnr_seq), np.mean(mse_seq), np.mean(ssim_seq), mean(gpu_memory_seq)]
